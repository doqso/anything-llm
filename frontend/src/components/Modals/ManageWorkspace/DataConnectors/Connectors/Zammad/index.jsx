...
export default function ZammadOptions() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      showToast(
        "Importing tickets from Zammad — this may take a while.",
        "info",
        { clear: true, autoClose: false }
      );
      const { data, error } = await System.dataConnectors.zammad.collect({
      ...
      showToast(
        `Tickets collected from Zammad. Output folder is ${data.destination}.`,
        "success",
        { clear: true }
      );
      e.target.reset();
      ...
      <div className="flex flex-col w-full px-1 md:pb-6 pb-16">
        <form className="w-full" onSubmit={handleSubmit}>
            ...
            <div className="w-full flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-y-1 mb-4">
                  <label className="text-white text-sm font-bold flex gap-x-2 items-center">
                    <p className="font-bold text-white">
                      {t("connectors.zammad.base_url")}
                    </p>
                  </label>
                  <p className="text-xs font-normal text-theme-text-secondary">
                    {t("connectors.zammad.base_url_explained")}
                  </p>
                </div>
                <input
                  type="url"
                  ...
                  placeholder="eg: https://zammad.example.com"
                  required={true}
                  autoComplete="off"
                  spellCheck={false}
                />
              ...
              <div className="flex flex-col pr-10">
                <div className="flex flex-col gap-y-1 mb-4">
                  <label className="text-white text-sm font-bold flex gap-x-2 items-center">
                    <p className="font-bold text-white">
                      {t("connectors.zammad.api_token")}
                    </p>
                  </label>
                  <p className="text-xs font-normal text-theme-text-secondary">
                    {t("connectors.zammad.api_token_explained")}
                  </p>
                </div>
                <input
                  type="password"
              ...
              <div className="flex flex-col pr-10">
                <div className="flex flex-col gap-y-1 mb-4">
                  <label className="text-white text-sm font-bold flex gap-x-2 items-center">
                    <p className="font-bold text-white">
                      {t("connectors.zammad.query")}
                    </p>
                  </label>
                  <p className="text-xs font-normal text-theme-text-secondary">
                    {t("connectors.zammad.query_explained")}
                  </p>
                </div>
                <textarea
                  name="query"
                  ...
                  <div className="flex flex-col pr-10">
                    <div className="flex flex-col gap-y-1 mb-4">
                      <label className="text-white text-sm font-bold flex gap-x-2 items-center cursor-pointer">
                        <Toggle
                          size="md"
                          enabled={includeInternal}
                          onChange={setIncludeInternal}
                        />
                        <p className="font-bold text-theme-text-primary">
                          {t("connectors.zammad.include_internal")}
                        </p>
                      </label>
                      <p className="text-xs font-normal text-theme-text-secondary">
                        {t("connectors.zammad.include_internal_explained")}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex flex-col gap-y-1 mb-4">
                      <label className="text-white text-sm font-bold flex gap-x-2 items-center cursor-pointer">
                        <Toggle
                          size="md"
                          enabled={bypassSSL}
                          onChange={setBypassSSL}
                        />
                        <p className="font-bold text-theme-text-primary">
                          {t("connectors.zammad.bypass_ssl")}
                        </p>
                      </label>
                      <p className="text-xs font-normal text-theme-text-secondary">
                        {t("connectors.zammad.bypass_ssl_explained")}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col gap-y-2 w-full pr-10">
            ...
            >
              {loading
                ? t("connectors.zammad.collecting")
                : t("connectors.zammad.submit")}
            </button>
          </div>
        </form>
      </div>
